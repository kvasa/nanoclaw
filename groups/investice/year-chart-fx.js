...
function ffill(map, dates) {
  const out = {};
  // předvyplnit poslední známou hodnotou z dat PŘED začátkem osy (kurz může mít jiné dny)
  const priorKeys = Object.keys(map).filter(k => k <= dates[0]).sort();
  let last = priorKeys.length ? map[priorKeys.at(-1)] : null;
...
async function run() {
  ...
  const config = {
    ...
    options: {
      ...
      plugins: {
        ...
        title: { display: true, text: 'Vývoj portfolia za poslední rok — vč. pohybu kurzu CZK',
  ...
  fs.writeFileSync('/workspace/group/year-chart-fx.png', await canvas.renderToBuffer(config));
