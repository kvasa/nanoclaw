...
export class SlackChannel implements Channel {
  ...
  private setupEventHandlers(): void {
    ...
    this.app.event('message', async ({ event }) => {
      ...
      // Handle file attachments: download to group folder so the agent can read them
  ...
  /**
   * Download Slack file attachments to the group folder and return descriptions
    ...
    for (const f of files) {
      ...
      try {
        ...
        if (contentLength > SLACK_FILE_MAX_BYTES) {
          logger.warn({ name, contentLength }, 'Slack file too large, skipping');
        ...
        if (buffer.length > SLACK_FILE_MAX_BYTES) {
          logger.warn({ name, size: buffer.length }, 'Slack file too large after download, skipping');
          descriptions.push(`[${typeLabel}: ${name} — too large to download]`);
          continue;
        }
